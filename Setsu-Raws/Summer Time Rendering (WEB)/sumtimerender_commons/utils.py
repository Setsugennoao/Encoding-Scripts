...
def merge_episodes(curr_ep_idx: int) -> vs.VideoNode:
    episodes = [EPS_SOURCES[curr_ep_idx]]
    EPISODE = episodes[0]
    OP_RANGES = EPS_OP_RANGES[curr_ep_idx]
    ED_RANGES = EPS_ED_RANGES[curr_ep_idx]

    if not any({OP_RANGES, ED_RANGES}):
        return EPISODE

    for i, ep, rop, red in zip(count(), EPS_SOURCES, EPS_OP_RANGES, EPS_ED_RANGES):
    ...
    if not len(episodes):
        return EPS_SOURCES[curr_ep_idx]
